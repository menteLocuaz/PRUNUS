...
func seedModulos(db *sql.DB) error {
	...
	OVERRIDING SYSTEM VALUE                          -- permite forzar el valor de SERIAL
	VALUES
	...
	-- Resincronizar la secuencia del SERIAL para que el próximo
	-- autoincremental arranque desde el valor máximo insertado (6).
	SELECT setval(
	...
	_, err = db.Exec(seed, idStatusActivo)
	return err
}
