...
func NewServiceCliente(s store.StoreCliente, logger *zap.Logger) *ServiceCliente {
...
func (s *ServiceCliente) GetClienteByID(ctx context.Context, id uuid.UUID) (*models.Cliente, error) {
	if id == uuid.Nil {
		zaplogger.WithContext(ctx, s.logger).Warn("Intento de obtener cliente con ID nulo")
...
func (s *ServiceCliente) CreateCliente(ctx context.Context, cliente models.Cliente) (*models.Cliente, error) {
	if cliente.NombreCompleto == "" {
		zaplogger.WithContext(ctx, s.logger).Warn("Intento de crear cliente sin nombre completo")
		return nil, errors.New("falta el nombre completo del cliente")
	}
	if cliente.IDStatus == uuid.Nil {
		zaplogger.WithContext(ctx, s.logger).Warn("Intento de crear cliente sin estatus", zap.String("nombre", cliente.NombreCompleto))
...
func (s *ServiceCliente) UpdateCliente(ctx context.Context, id uuid.UUID, cliente models.Cliente) (*models.Cliente, error) {
	if id == uuid.Nil {
		zaplogger.WithContext(ctx, s.logger).Warn("Intento de actualizar cliente con ID nulo")
		return nil, errors.New("el ID del cliente es requerido")
	}
	if cliente.NombreCompleto == "" {
		zaplogger.WithContext(ctx, s.logger).Warn("Intento de actualizar cliente con nombre vacío", zap.String("id", id.String()))
		return nil, errors.New("falta el nombre completo del cliente")
	}
...
func (s *ServiceCliente) DeleteCliente(ctx context.Context, id uuid.UUID) error {
	if id == uuid.Nil {
		zaplogger.WithContext(ctx, s.logger).Warn("Intento de eliminar cliente con ID nulo")
