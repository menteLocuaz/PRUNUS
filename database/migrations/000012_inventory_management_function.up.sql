-- 1. FUNCIÓN PARA GESTIÓN MASIVA DE MOVIMIENTOS DE INVENTARIO
-- Permite procesar un array JSON de productos de forma atómica.
...
BEGIN
    ...
    -- ── 2. Validación preventiva de stock (Solo para salidas) ────────
    -- Bloqueamos las filas para evitar que otra transacción cambie el stock durante el proceso.
    IF v_es_salida THEN
    ...
    -- El trigger 'trg_actualizar_stock_movimiento' (de la migración 000006)
    -- actualizará automáticamente las tablas 'inventario' y 'producto'.
    FOR v_item IN SELECT * FROM jsonb_to_recordset(p_items_json) AS x(id_producto UUID, cantidad NUMERIC)
    LOOP
